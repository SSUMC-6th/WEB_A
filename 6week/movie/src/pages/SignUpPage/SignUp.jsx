...
import { StyledButton, StyledTitle, StyledWrapper, StyledInput, ErrorMessage, LoginLink } from "./SignUp.style";
  ...
  const validateField = (field, value) => {
    ...
    switch (field) {
      ...
      case 'email':
        ...
        break;
      case 'age':
        errMsg = !value ? '나이를 입력해주세요.' :
                 isNaN(ageVal) || ageVal !== parseFloat(value) ? '나이는 정수여야 합니다.' :
                 ageVal < 0 ? '나이는 음수가 될 수 없습니다.' :
                 ageVal < 19 ? '19살 미만은 가입이 불가능합니다.' : '';
        break;
      case 'password':
        errMsg = !value ? '비밀번호를 입력해주세요.' :
                 value.length < 4 ? '비밀번호는 최소 4자리 이상이어야 합니다.' :
                 value.length > 12 ? '비밀번호는 최대 12자리까지 가능합니다.' :
                 !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{4,}$/.test(value) ? '영어 대소문자, 숫자, 특수문자를 모두 포함해야 합니다.' : '';
        break;
